...
public class Parse_18_Swenk_Stelididae_1914 {
    ...
    private HierarchyEntry genHierarchyEntry(String taxonNameInfo) throws IOException {
        ...
        String rankRemoved = TaxonXMLCommon.removeFirstRank(taxonName);
        ...
        for(Paragraph paragraph : paragraphs) {
            switch(paragraph.getType()) {
                ...
                case PARAGRAPH_TAXONNAME:
                {
                    ...
                    // set metadata
                    TaxonXMLCommon.addNewMeta(taxonomy, this.processor_name, document.getFilename());
                    String rankRemovedName = TaxonXMLCommon.removeFirstRank(getTaxonName(paragraph.getContent()));
                ...
                case PARAGRAPH_KEY:
                {
                    if(paragraph.getContent().startsWith("KEY TO THE NEBRASKA SPECIES")) {
                        prevTitle = paragraph.getContent();
                    } else if(paragraph.getContent().startsWith("KEY TO")) {
                        prevTitle = null;
