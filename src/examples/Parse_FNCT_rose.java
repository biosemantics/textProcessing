...
public class Parse_FNCT_rose {
    ...
    public Parse_FNCT_rose(String processor_name) {
    ...
    private String getAuthority(String name) {
        ...
        int daIdx1 = name2.indexOf(" (F. Michx.) Fernald");
        if (daIdx1 >= 0) {
            startPos = Math.min(startPos, daIdx1 + 3);
        }
        
        int daIdx2 = name2.indexOf(" Sarg. & Engelm.");
        if (daIdx2 >= 0) {
            startPos = Math.min(startPos, daIdx2 + 3);
        }
        
        int daIdx3 = name2.indexOf(" (Vilm. ex J. Gay) Staudt");
        if (daIdx3 >= 0) {
            startPos = Math.min(startPos, daIdx3 + 3);
        }
        
        int daIdx4 = name2.indexOf(" (Rydb.) Fernald & Weath.");
        if (daIdx4 >= 0) {
            startPos = Math.min(startPos, daIdx4 + 3);
        }
        
        int daIdx5 = name2.indexOf(" Fernald & Weath.");
        if (daIdx5 >= 0) {
            startPos = Math.min(startPos, daIdx5 + 3);
        }
        
        int daIdx6 = name2.indexOf(" (Muhl. ex Willd.) Britton");
        if (daIdx6 >= 0) {
            startPos = Math.min(startPos, daIdx6 + 3);
        }
        
        int daIdx7 = name2.indexOf(" Engelm. & A. Gray");
        if (daIdx7 >= 0) {
            startPos = Math.min(startPos, daIdx7 + 3);
        }
        
        int daIdx8 = name2.indexOf(" W. Wight & Hedrick");
        if (daIdx8 >= 0) {
            startPos = Math.min(startPos, daIdx8 + 2);
        }
        
        int daIdx9 = name2.indexOf(" (Greene) Cockerell");
        if (daIdx9 >= 0) {
            startPos = Math.min(startPos, daIdx9 + 2);
        }
        
        int daIdx10 = name2.indexOf(" Nutt. ex Torr. & A. Gray");
        if (daIdx10 >= 0) {
            startPos = Math.min(startPos, daIdx10 + 2);
        }
        
        int daIdx11 = name2.indexOf(" Borrer ex Sm.");
        if (daIdx11 >= 0) {
            startPos = Math.min(startPos, daIdx11 + 2);
        }
        
        int daIdx12 = name2.indexOf(" Thunb. ex Murr.");
        if (daIdx12 >= 0) {
            startPos = Math.min(startPos, daIdx12 + 2);
        }
        
        int daIdx13 = name2.indexOf(" Torr. & A. Gray");
        if (daIdx13 >= 0) {
            startPos = Math.min(startPos, daIdx13 + 2);
        }
        
        int daIdx14 = name2.indexOf(" Vest ex Tratt.");
        if (daIdx14 >= 0) {
            startPos = Math.min(startPos, daIdx14 + 2);
        }
        
        int daIdx15 = name2.indexOf(" (Nutt. ex Hook.) Torr. & A. Gray");
        if (daIdx15 >= 0) {
            startPos = Math.min(startPos, daIdx15 + 2);
        }
    ...
    private HierarchyEntry getHierarchyEntry(String name, String rank) throws IOException {
        ...
        rank_relations.add(new RankRelation("Species", "Variety"));
        rank_relations.add(new RankRelation("Species", "Subspecies"));
        
        String firstPart = null;
        boolean hasSecondPart = false;
        if (name.indexOf(" ssp. ") >= 0 || name.indexOf(" var. ") >= 0 || name.indexOf(" subsp. ") >= 0) {
            // split into two
            int idx = 0;
            int len = 0;
            String rnk = null;
            if (name.indexOf(" ssp. ") >= 0) {
                ...
                idx = name.indexOf(" subsp. ");
                len = 8;
                rnk = "Subspecies";
            }

            firstPart = name.substring(0, idx).trim();
            String secondPart = name.substring(idx + len).trim();
    ...
    private HierarchyEntry genHierarchyEntry(String taxonNameInfo) throws IOException {
        ...
        if (rank == null) {
            ...
            if (taxonNameInfo.equals("ROSACEAE")) {
                rank = "Family";
            } if (taxonNameInfo.equals(taxonNameInfo.toUpperCase()) && taxonNameInfo.indexOf(" ") < 0) {
                rank = "Genus";
            } else {
        ...
        for (Paragraph paragraph : paragraphs) {
            switch (paragraph.getType()) {
                case PARAGRAPH_TITLE: {
                    HierarchyEntry hierarchy_entry_rosaceae = getHierarchyEntry("ROSACEAE", "Family");
                    hierarchy.addEntry(hierarchy_entry_rosaceae);
                    break;
                }
                case PARAGRAPH_TAXONNAME: {
                    String taxonname = paragraph.getContent();
                    if(taxonname.equals("Mature larva of Melanomada annectens Snelling and Rozen")) {
                        taxonname = "Melanomada annectens Snelling and Rozen";
                    }
    ...
    public static void main(String[] args) throws Exception {
        Parse_FNCT_rose obj = new Parse_FNCT_rose("Illyoung Choi");
        obj.start(32);
    }
}
