 ...
 * @author iychoi
 */
public class Parse_Castanea_Rubus_98 {
    ...
    public Parse_Castanea_Rubus_98(String processor_name) {
    ...
    private String getTaxonName(String nameInfo) {
        ...
        int idxsDot = newTaxon.indexOf(".");
        if (idxsDot > 0) {
            newTaxon = newTaxon.substring(idxsDot + 1).trim();
        }
        
        int idxeDot = newTaxon.indexOf(",");
        if (idxeDot > 0) {
            newTaxon = newTaxon.substring(0, idxeDot).trim();
        }
        
        if (newTaxon.startsWith("R. ")) {
            newTaxon = "Rubus " + newTaxon.substring(3);
        }
    ...
    private String getAuthority(String name) {
        ...
        } else if (name.endsWith("Porter ex L.H. Bailey")) {
            startPos = startPos - 16;
        } else if (name.endsWith("L.H. Bailey")) {
            startPos = startPos - 9;
        } else if (name.endsWith("(Michx.) Maxim.")) {
            startPos = startPos - 10;
        } else if (name.endsWith("(L.H. Bailey) Rydb. in Britton")) {
    ...
    private String parentRank = null;
    private HierarchyEntry getHierarchyEntry(String name, String rank) throws IOException {
        ...
        if(parentRank == null || parentRank.equals("Subgenus")) {
            ...
            rank_relations.add(new RankRelation("Subgenus", "Section"));
            rank_relations.add(new RankRelation("Species", "Subspecies"));
            rank_relations.add(new RankRelation("Species", "Variety"));
        } else if(parentRank.equals("Section")) {
            rank_relations.add(new RankRelation("Order", "Order"));
            rank_relations.add(new RankRelation("Order", "Superfamily"));
            rank_relations.add(new RankRelation("Superfamily", "Family"));
            rank_relations.add(new RankRelation("Family", "Genus"));
            rank_relations.add(new RankRelation("Genus", "Subgenus"));
        ...
        if(rank.equals("Subgenus") || rank.equals("Section")) {
            parentRank = rank;
        }
        ...
        if (firstPart.indexOf(" ") < 0) {
            ...
            if(firstName.equals("Rubus x neglectus")) {
                name_parts = new String[2];
                name_parts[0] = "Rubus";
                name_parts[1] = "x neglectus";
    ...
    private HierarchyEntry genHierarchyEntry(String taxonNameInfo) throws IOException {
        ...
        if (rank == null) {
            ...
            if (rankRemoved.endsWith(" n. sp.")) {
                ...
                rank = "Variety";
            } else if (rankRemoved.indexOf(" var. ") >= 0) {
                rank = "Variety";
        ...
        for (Paragraph paragraph : paragraphs) {
            switch (paragraph.getType()) {
                case PARAGRAPH_TITLE: {
                    ...
                    HierarchyEntry hierarchy_entry_nomada = getHierarchyEntry("Rubus", "Genus");
                    ...
                    taxonomy.setTaxonName("Rubus");
                    
                    HierarchyEntry hierarchy_entry = getHierarchyEntry("Rubus", "Genus");
    ...
    public static void main(String[] args) throws Exception {
        Parse_Castanea_Rubus_98 obj = new Parse_Castanea_Rubus_98("Illyoung Choi");
        obj.start(27);
    }
}
