 ...
 * @author iychoi
 */
public class Parse_11_MitchellPart_I_Andrenidae_1960 {
    ...
    public Parse_11_MitchellPart_I_Andrenidae_1960(String processor_name) {
    ...
    private String getTaxonName(String nameInfo) {
        ...
        int idxNewSubsp = newTaxon.indexOf(" new subspecies");
        if (idxNewSubsp > 0) {
            newTaxon = newTaxon.substring(0, idxNewSubsp).trim();
        }
    ...
    private String getAuthority(String name) {
        ...
        if (name2.endsWith("Dalla Torre")) {
            startPos = startPos - 6;
        } else if (name2.endsWith("(Dalla Torre)")) {
            startPos = startPos - 7;
    ...
    private HierarchyEntry genHierarchyEntry(String taxonNameInfo) throws IOException {
        ...
        if (rank == null) {
            ...
            if(rankRemoved.equals("Colletes robertsonii Dalla Torre")) {
                ...
                rank = "Species";
            } else if (taxonNameInfo.endsWith(" n. sp.")) {
                rank = "Species";
            } else if (taxonNameInfo.endsWith(" n. subsp.")) {
                rank = "Subspecies";
            } else if (taxonNameInfo.endsWith(" var.")) {
                rank = "Variety";
            } else if (taxonNameInfo.endsWith(" new subgenus")) {
                rank = "Subgenus";
            } else if (taxonNameInfo.endsWith(" new species")) {
                rank = "Species";
            } else if (taxonNameInfo.endsWith(" new subspecies")) {
                rank = "Subspecies";
                ...
                if(rankRemoved.endsWith("Dalla Torre") || rankRemoved.endsWith("(Dalla Torre)")) {
                    rank = "Species";    
                } else {
                    rank = "Subspecies";    
                }
                ...
                rank = "Subspecies";
            } else if (rankRemoved.split("\\s").length == 6) {
                if(rankRemoved.endsWith("Viereck and Cockerell")) {
                    rank = "Species";
                } else {
                    rank = "Subspecies";
                }
        ...
        for (Paragraph paragraph : paragraphs) {
            switch (paragraph.getType()) {
                case PARAGRAPH_TITLE:
                {
                    ...
                    taxonomy.setTaxonName("ANDRENIDAE");
                    
                    HierarchyEntry hierarchy_entry = getHierarchyEntry("ANDRENIDAE", "Family");
    ...
    public static void main(String[] args) throws Exception {
        Parse_11_MitchellPart_I_Andrenidae_1960 obj = new Parse_11_MitchellPart_I_Andrenidae_1960("Illyoung Choi");
        obj.start(30);
    }
}
